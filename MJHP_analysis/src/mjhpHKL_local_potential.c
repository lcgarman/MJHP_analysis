...
{ 
  ...
  /*Allocate memory for variables in this function*/
  LOCAL->rec_grid = NULL;
  LOCAL->rec_grid = AllocateMemory_threeD_complex(LOCAL->rec_grid, ngfftx, ngffty, ngfftz);
...
//		  occ = WFK->occ[kptno][band];
          ...
          /*store potential energy contributions*/
          LOCAL->rec_grid[delta_h][delta_k][delta_l] = gsl_complex_add(LOCAL->rec_grid[delta_h][delta_k][delta_l], potentialE);  
		  total_local += GSL_REAL(potentialE);
        } /*END band->nband loop*/
	  } /*END pw2->npw loop*/
	} /*END pw1->npw loop*/
