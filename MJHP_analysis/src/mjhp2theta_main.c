...
int main(int argc, char * argv[])
{
  ...
  mjhp2theta_local_potential(flog, &tth, &estp, &grid, &wave, &ucell, &bin, &econ);
  ...
  mjhp2theta_nonlocal_potential(flog, &tth, &estp, &wave, &psp, &ucell, &atom, &econ);

  /*combine nonlocal and local potential energy*/
  concatinate_twotheta_potential(flog, &econ, &estp, &tth); 
